...
Imports Community.VisualBasic.CompilerServices.StructUtils
...
Namespace Global.Community.VisualBasic.CompilerServices
  ...
  Friend NotInheritable Class PutHandler
    ...
    Sub New(oFile As VB6File)
      MyBase.New()
      m_oFile = oFile
    End Sub

    Function Callback(field_info As Reflection.FieldInfo, ByRef vValue As Object) As Boolean Implements IRecordEnum.Callback
      ...
      If FieldType.IsArray() Then
        ...
        If attributeList IsNot Nothing AndAlso attributeList.Length <> 0 Then
  ...
  Friend NotInheritable Class GetHandler
    ...
    Function Callback(field_info As Reflection.FieldInfo, ByRef vValue As Object) As Boolean Implements IRecordEnum.Callback
      ...
      If FieldType.IsArray() Then
        ...
        If FixedStringAttributeList IsNot Nothing AndAlso FixedStringAttributeList.Length > 0 Then
  ...
  Friend MustInherit Class VB6File
    ...
    Protected Sub New(sPath As String, access As OpenAccess, share As OpenShare, lRecordLen As Integer)
      MyBase.New()
    ...
    Friend Sub SetColumn(lColumn As Integer)
    ...
    Friend Sub SetWidth(RecordWidth As Integer)
    ...
    ' Output functions.
    '============================================================================
    Friend Overridable Sub WriteLine(s As String)
      Throw VbMakeException(vbErrors.BadFileMode)
    End Sub

    Friend Overridable Sub WriteString(s As String)
      Throw VbMakeException(vbErrors.BadFileMode)
    End Sub
    ...
    Friend Sub SetRecord(RecordNumber As Long)
    ...
    Friend Overridable Overloads Sub Seek(BaseOnePosition As Long)
    ...
    Friend Sub SeekOffset(offset As Long)
    ...
    Friend Overridable Overloads Sub Lock(Record As Long)
      If m_lRecordLen = -1 Then
    ...
    Friend Overridable Overloads Sub Unlock(Record As Long)
      If m_lRecordLen = -1 Then
    ...
    Friend Overridable Overloads Sub Lock(RecordStart As Long, RecordEnd As Long)
      If m_lRecordLen = -1 Then
    ...
    Friend Overridable Overloads Sub Unlock(RecordStart As Long, RecordEnd As Long)
      If m_lRecordLen = -1 Then
    ...
    Protected Overridable Function InputNum(vt As VariantType) As Object
    ...
    Friend Function InputString(lLen As Integer) As String
    ...
    Friend Sub SPC(iCount As Integer)
...
NewLine:
    ...
    Friend Sub Tab(Column As Integer)
      Dim lCurPos As Integer
      Dim lWidth As Integer
      Dim s As String
    ...
    Friend Shared Function VTType(VarName As Object) As VT
    ...
    Friend Shared Function VTFromComType(typ As System.Type) As VT
    ...
    Friend Sub PutFixedArray(RecordNumber As Long, arr As System.Array, ElementType As System.Type,
        Optional FixedStringLength As Integer = -1, Optional FirstBound As Integer = -1,
        Optional SecondBound As Integer = -1)
    ...
    Friend Sub PutDynamicArray(RecordNumber As Long, arr As System.Array,
        Optional ContainedInVariant As Boolean = True, Optional FixedStringLength As Integer = -1)
    ...
    Friend Sub LengthCheck(Length As Integer)
      If m_lRecordLen = -1 Then
        Exit Sub
      End If
    ...
    Friend Sub PutFixedLengthString(RecordNumber As Long, s As String, lengthToWrite As Integer)
      ...
      If ByteLength < lengthToWrite Then
        s &= StrDup(lengthToWrite - ByteLength, PadChar)
      End If
    ...
    Friend Sub PutVariantString(RecordNumber As Long, s As String)
      If s Is Nothing Then
        s = ""
      End If
    ...
    Friend Sub PutString(RecordNumber As Long, s As String)
      If s Is Nothing Then
        s = ""
      End If
    ...
    Friend Sub PutStringWithLength(RecordNumber As Long, s As String)
      If s Is Nothing Then
        s = ""
      End If
    ...
    Friend Sub PutDate(RecordNumber As Long, dt As Date, Optional ContainedInVariant As Boolean = False)
    ...
    Friend Sub PutShort(RecordNumber As Long, i As Short, Optional ContainedInVariant As Boolean = False)
    ...
    Friend Sub PutInteger(RecordNumber As Long, l As Integer, Optional ContainedInVariant As Boolean = False)
    ...
    Friend Sub PutLong(RecordNumber As Long, l As Long, Optional ContainedInVariant As Boolean = False)
      Dim RecLength As Integer = 8
    ...
    Friend Sub PutByte(RecordNumber As Long, byt As Byte, Optional ContainedInVariant As Boolean = False)
    ...
    Friend Sub PutChar(RecordNumber As Long, ch As Char, Optional ContainedInVariant As Boolean = False)
      Dim RecLength As Integer = 2
    ...
    Friend Sub PutSingle(RecordNumber As Long, sng As Single, Optional ContainedInVariant As Boolean = False)
      Dim RecLength As Integer = 4
    ...
    Friend Sub PutDouble(RecordNumber As Long, dbl As Double, Optional ContainedInVariant As Boolean = False)
      Dim RecLength As Integer = 8
    ...
    Friend Sub PutEmpty(RecordNumber As Long)
    ...
    Friend Sub PutBoolean(RecordNumber As Long, b As Boolean, Optional ContainedInVariant As Boolean = False)
      Dim RecLength As Integer = 2
    ...
    Friend Sub PutDecimal(RecordNumber As Long, dec As Decimal, Optional ContainedInVariant As Boolean = False)
    ...
    Friend Sub PutCurrency(RecordNumber As Long, dec As Decimal, Optional ContainedInVariant As Boolean = False)
      Dim RecLength As Integer = 16
    ...
    Friend Sub PutRecord(RecordNumber As Long, o As ValueType)
    ...
    Friend Function ComTypeFromVT(vtype As VT) As System.Type
      Select Case vtype
        Case VT.Variant
    ...
    Friend Sub GetFixedArray(RecordNumber As Long, ByRef arr As System.Array,
FieldType As System.Type, Optional FirstBound As Integer = -1,
        Optional SecondBound As Integer = -1, Optional FixedStringLength As Integer = -1)
    ...
    Friend Sub GetDynamicArray(ByRef arr As System.Array, t As System.Type, Optional FixedStringLength As Integer = -1)
      arr = GetArrayDesc(t)
    ...
    Private Sub PutArrayDesc(arr As System.Array)
      Dim cDims As Short
      Dim i As Integer
    ...
    Friend Function GetArrayDesc(typ As System.Type) As System.Array
      Dim cDims As Integer
    ...
    Friend Overridable Function GetLengthPrefixedString(RecordNumber As Long) As String
      SetRecord(RecordNumber)
    ...
    Friend Overridable Function GetFixedLengthString(RecordNumber As Long, ByteLength As Integer) As String
      SetRecord(RecordNumber)
      Return ReadString(ByteLength)
    End Function

    Protected Overloads Function ReadString(ByteLength As Integer) As String
    ...
    Friend Function GetDate(RecordNumber As Long) As Date
      Dim dbl As Double
    ...
    Friend Function GetShort(RecordNumber As Long) As Short
      Dim s As Short
    ...
    Friend Function GetInteger(RecordNumber As Long) As Integer
      Dim i As Integer
    ...
    Friend Function GetLong(RecordNumber As Long) As Long
      Dim l As Long
    ...
    Friend Function GetByte(RecordNumber As Long) As Byte
      Dim b As Byte
    ...
    Friend Function GetChar(RecordNumber As Long) As Char
      Dim c As Char
    ...
    Friend Function GetSingle(RecordNumber As Long) As Single
      Dim s As Single
    ...
    Friend Function GetDouble(RecordNumber As Long) As Double
      Dim d As Double
    ...
    Friend Function GetDecimal(RecordNumber As Long) As Decimal
      Dim vt As Integer
      Dim lo, mid, hi As Integer
      Dim flags As Byte
    ...
    Friend Function GetCurrency(RecordNumber As Long) As Decimal
      Dim i64 As Int64
    ...
    Friend Function GetBoolean(RecordNumber As Long) As Boolean
      Dim i As Short
    ...
    Friend Sub GetRecord(RecordNumber As Long, ByRef o As ValueType, Optional ContainedInVariant As Boolean = False)
    ...
    Friend Sub PutArrayData(arr As System.Array, typ As System.Type, FixedStringLength As Integer,
FirstBound As Integer, SecondBound As Integer)
              ...
              Else
                ...
                If FixedStringLength > 0 AndAlso ByteLength > FixedStringLength Then
                  ...
                  If ByteLength = sTemp.Length Then
                    ...
                    sTemp = Community.VisualBasic.Left(sTemp, FixedStringLength)
    ...
    Friend Sub GetArrayData(arr As System.Array, typ As System.Type, Optional FirstBound As Integer = -1,
        Optional SecondBound As Integer = -1, Optional FixedStringLength As Integer = -1)
    ...
    ''' </summary>
    Private Function GetByteLength(vtype As VT) As Integer
      Select Case vtype
        Case VT.Byte    '1 byte
          Return 1
        Case VT.Short '2 bytes
    ...
    Private Sub PrintTab(ti As TabInfo)
      If ti.Column = -1 Then
    ...
    Friend Sub PrintLine(ParamArray Output() As Object)
      Print(Output)
      WriteLine(Nothing)
    End Sub

    Friend Sub Print(ParamArray Output() As Object)
      Dim i As Integer
      Dim s As String
      Dim obj As Object
      Dim typ As Type
    ...
    Friend Sub WriteLineHelper(ParamArray Output() As Object)
      InternalWriteHelper(Output)
      WriteLine(Nothing)
    End Sub

    Friend Sub WriteHelper(ParamArray Output() As Object)
      InternalWriteHelper(Output)
      WriteString(",")
    End Sub

    Private Sub InternalWriteHelper(ParamArray Output() As Object)
    ...
    Private Function IOStrFromSingle(Value As Single, NumberFormat As NumberFormatInfo) As String
      Return Value.ToString(Nothing, NumberFormat)
    End Function

    Private Function IOStrFromDouble(Value As Double, NumberFormat As NumberFormatInfo) As String
      Return Value.ToString(Nothing, NumberFormat)
    End Function

    Private Function IOStrFromDecimal(Value As Decimal, NumberFormat As NumberFormatInfo) As String
      Return Value.ToString("G29", NumberFormat)
    End Function

    Friend Function FormatUniversalDate(dt As Date) As String
    ...
    Protected Function GetQuotedString(Value As String) As String
      'Wrap Value with quotes, but make sure to escape quotes contained in Value.
      Return """" & Value.Replace("""", """""") & """"
    End Function

    Protected Sub ValidateRec(RecordNumber As Long)
      If RecordNumber < 1 Then
        Throw VbMakeException(vbErrors.BadRecordNum)
      End If
    End Sub

    Friend Overridable Sub GetObject(ByRef Value As Object, Optional RecordNumber As Long = 0, Optional ContainedInVariant As Boolean = True)
      Throw VbMakeException(vbErrors.BadFileMode)
    End Sub

    Friend Overridable Overloads Sub [Get](ByRef Value As ValueType, Optional RecordNumber As Long = 0)
      Throw VbMakeException(vbErrors.BadFileMode)
    End Sub

    Friend Overridable Overloads Sub [Get](ByRef Value As System.Array, Optional RecordNumber As Long = 0,
        Optional ArrayIsDynamic As Boolean = False, Optional StringIsFixedLength As Boolean = False)

      Throw VbMakeException(vbErrors.BadFileMode)
    End Sub

    Friend Overridable Overloads Sub [Get](ByRef Value As Boolean, Optional RecordNumber As Long = 0)
      Throw VbMakeException(vbErrors.BadFileMode)
    End Sub

    Friend Overridable Overloads Sub [Get](ByRef Value As Byte, Optional RecordNumber As Long = 0)
      Throw VbMakeException(vbErrors.BadFileMode)
    End Sub

    Friend Overridable Overloads Sub [Get](ByRef Value As Short, Optional RecordNumber As Long = 0)
      Throw VbMakeException(vbErrors.BadFileMode)
    End Sub

    Friend Overridable Overloads Sub [Get](ByRef Value As Integer, Optional RecordNumber As Long = 0)
      Throw VbMakeException(vbErrors.BadFileMode)
    End Sub

    Friend Overridable Overloads Sub [Get](ByRef Value As Long, Optional RecordNumber As Long = 0)
      Throw VbMakeException(vbErrors.BadFileMode)
    End Sub

    Friend Overridable Overloads Sub [Get](ByRef Value As Char, Optional RecordNumber As Long = 0)
      Throw VbMakeException(vbErrors.BadFileMode)
    End Sub

    Friend Overridable Overloads Sub [Get](ByRef Value As Single, Optional RecordNumber As Long = 0)
      Throw VbMakeException(vbErrors.BadFileMode)
    End Sub

    Friend Overridable Overloads Sub [Get](ByRef Value As Double, Optional RecordNumber As Long = 0)
      Throw VbMakeException(vbErrors.BadFileMode)
    End Sub

    Friend Overridable Overloads Sub [Get](ByRef Value As Decimal, Optional RecordNumber As Long = 0)
      Throw VbMakeException(vbErrors.BadFileMode)
    End Sub

    Friend Overridable Overloads Sub [Get](ByRef Value As String, Optional RecordNumber As Long = 0, Optional StringIsFixedLength As Boolean = False)
      Throw VbMakeException(vbErrors.BadFileMode)
    End Sub

    Friend Overridable Overloads Sub [Get](ByRef Value As Date, Optional RecordNumber As Long = 0)
      Throw VbMakeException(vbErrors.BadFileMode)
    End Sub

    Friend Overridable Sub PutObject(Value As Object, Optional RecordNumber As Long = 0, Optional ContainedInVariant As Boolean = True)
      Throw VbMakeException(vbErrors.BadFileMode)
    End Sub

    Friend Overridable Overloads Sub Put(Value As Object, Optional RecordNumber As Long = 0)
      Throw VbMakeException(vbErrors.BadFileMode)
    End Sub

    Friend Overridable Overloads Sub Put(Value As ValueType, Optional RecordNumber As Long = 0)
      Throw VbMakeException(vbErrors.BadFileMode)
    End Sub

    Friend Overridable Overloads Sub Put(Value As System.Array, Optional RecordNumber As Long = 0,
        Optional ArrayIsDynamic As Boolean = False, Optional StringIsFixedLength As Boolean = False)

      Throw VbMakeException(vbErrors.BadFileMode)
    End Sub

    Friend Overridable Overloads Sub Put(Value As Boolean, Optional RecordNumber As Long = 0)
      Throw VbMakeException(vbErrors.BadFileMode)
    End Sub

    Friend Overridable Overloads Sub Put(Value As Byte, Optional RecordNumber As Long = 0)
      Throw VbMakeException(vbErrors.BadFileMode)
    End Sub

    Friend Overridable Overloads Sub Put(Value As Short, Optional RecordNumber As Long = 0)
      Throw VbMakeException(vbErrors.BadFileMode)
    End Sub

    Friend Overridable Overloads Sub Put(Value As Integer, Optional RecordNumber As Long = 0)
      Throw VbMakeException(vbErrors.BadFileMode)
    End Sub

    Friend Overridable Overloads Sub Put(Value As Long, Optional RecordNumber As Long = 0)
      Throw VbMakeException(vbErrors.BadFileMode)
    End Sub

    Friend Overridable Overloads Sub Put(Value As Char, Optional RecordNumber As Long = 0)
      Throw VbMakeException(vbErrors.BadFileMode)
    End Sub

    Friend Overridable Overloads Sub Put(Value As Single, Optional RecordNumber As Long = 0)
      Throw VbMakeException(vbErrors.BadFileMode)
    End Sub

    Friend Overridable Overloads Sub Put(Value As Double, Optional RecordNumber As Long = 0)
      Throw VbMakeException(vbErrors.BadFileMode)
    End Sub

    Friend Overridable Overloads Sub Put(Value As Decimal, Optional RecordNumber As Long = 0)
      Throw VbMakeException(vbErrors.BadFileMode)
    End Sub

    Friend Overridable Overloads Sub Put(Value As String, Optional RecordNumber As Long = 0, Optional StringIsFixedLength As Boolean = False)
      Throw VbMakeException(vbErrors.BadFileMode)
    End Sub

    Friend Overridable Overloads Sub Put(Value As Date, Optional RecordNumber As Long = 0)
      Throw VbMakeException(vbErrors.BadFileMode)
    End Sub
...
SkipWhiteSpaceExit:
    ...
    Private Function GetFileInTerm(iTermType As Short) As String
    ...
    Protected Function IntlIsSpace(lch As Integer) As Boolean
      ' consider testing for intl spaces
      Return (lch = lchSpace) Or (lch = lchIntlSpace)
    End Function

    Protected Function IntlIsDoubleQuote(lch As Integer) As Boolean
      ' consider testing for intl double quotes
      Return (lch = lchDoubleQuote)
    End Function

    Protected Function IntlIsComma(lch As Integer) As Boolean
    ...
    Protected Function ReadInField(iTermType As Short) As String
    ...
    Protected Function CheckEOF(lChar As Integer) As Boolean

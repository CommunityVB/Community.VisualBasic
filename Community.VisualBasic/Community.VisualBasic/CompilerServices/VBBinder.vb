...
Imports Community.VisualBasic.CompilerServices.LateBinding
...
Namespace Global.Community.VisualBasic.CompilerServices
  ...
  Friend NotInheritable Class VBBinder
    ...
    Private Sub ThrowInvalidCast(ArgType As System.Type, ParmType As System.Type, ParmIndex As Integer)
    ...
    Private ReadOnly m_ByRefFlags As Boolean()

    Sub New(CopyBack As Boolean())
    ...
    Public Overrides Sub ReorderArgumentArray(ByRef args() As Object, objState As Object)
      ...
      If state.m_OriginalParamOrder IsNot Nothing Then

        'The arguments have been reordered, lets put them back
        If (m_ByRefFlags IsNot Nothing) Then
...
CleanupAndExit:
    ...
    Public Overrides Function BindToMethod(bindingAttr As BindingFlags, match() As MethodBase, ByRef args() As Object, modifiers() As ParameterModifier, culture As CultureInfo, names() As String, ByRef ObjState As Object) As MethodBase
      ...
      If (m_CachedMember IsNot Nothing) AndAlso
         (m_CachedMember.MemberType = MemberTypes.Method) AndAlso
         (match(0) IsNot Nothing) AndAlso
          ...
          ElseIf ThisMethod.IsVirtual Then
            ...
            If (ThisMethod.Attributes And MethodAttributes.NewSlot) <> 0 Then
              ...
              For j = 0 To match.GetUpperBound(0)
                ...
                If MethodIndex <> j AndAlso (match(j) IsNot Nothing) AndAlso
      ...
      If names IsNot Nothing Then
        ...
        For MethodIndex = 0 To match.GetUpperBound(0)
          ...
          If ThisMethod IsNot Nothing Then
      ...
      For ArgIndex = 0 To args.GetUpperBound(0)
        If args(ArgIndex) IsNot Nothing Then
                ...
                If ParmType Is GetType(Char()) Then
                  Select Case ArgTypeCode
                    ...
                    Case TypeCode.Object
                      If ArgType IsNot GetType(Char()) Then
                        GoTo ClearMethod7
                      End If
...
NextMethod8:
          ...
          For MethodIndex = 0 To match.GetUpperBound(0)
            If match(MethodIndex) Is Nothing Then
              ...
              For j = MethodIndex + 1 To match.GetUpperBound(0)
                TmpMatch = match(j)
                If TmpMatch IsNot Nothing Then
      ...
      '
      Debug.Assert(SelectedMatch IsNot Nothing, "Should have already thrown an exception")
        ...
        ElseIf (ParamArrayExpanded1 AndAlso ParamArrayIndex1 <> PARAMARRAY_NONE AndAlso ParmIndex >= ParamArrayIndex1) Then
          ...
          If ParamArrayElementType1 Is Type0 Then
            ...
            If MostSpecific = -1 AndAlso ParamArrayIndex0 = PARAMARRAY_NONE AndAlso ParmIndex = LastNonSetValueIndex0 AndAlso
                (args(LastNonSetValueIndex0) IsNot Nothing) Then
              MostSpecific = 0
            End If
        ...
        ElseIf (ParamArrayExpanded0 AndAlso ParamArrayIndex0 <> PARAMARRAY_NONE AndAlso ParmIndex >= ParamArrayIndex0) Then
          ...
          If ParamArrayElementType0 Is Type1 Then
            ...
            If MostSpecific = -1 AndAlso ParamArrayIndex1 = PARAMARRAY_NONE AndAlso ParmIndex = LastNonSetValueIndex1 AndAlso
                (args(LastNonSetValueIndex1) IsNot Nothing) Then
              MostSpecific = 1
            End If
    ...
    Private Function MatchesParamArraySignature(param0 As ParameterInfo(), param1 As ParameterInfo(), ParamArrayIndex1 As Integer, IsPropertySet As Boolean, ArgCountUpperBound As Integer) As Boolean
      ...
      For i = 0 To UpperBound
        ...
        If paramType0 IsNot paramType1 Then
          Return False
        End If
      Next i
    ...
    Private Function MethodsDifferOnlyByReturnType(match1 As MethodBase, match2 As MethodBase) As Boolean
      ...
      For i = 0 To UpperBound
        ...
        End If
        If paramType1 IsNot paramType2 Then
          Return False
        End If
      Next i
    ...
    Public Overrides Function BindToField(bindingAttr As BindingFlags, match() As FieldInfo, value As Object, culture As CultureInfo) As FieldInfo
      ...
      If (m_CachedMember IsNot Nothing) AndAlso (m_CachedMember.MemberType = MemberTypes.Field) AndAlso (match(0) IsNot Nothing) AndAlso
          (match(0).Name = m_CachedMember.Name) Then
    ...
    Friend Function InvokeMember(name As String,
invokeAttr As BindingFlags,
objType As System.Type,
objIReflect As IReflect,
target As Object,
args As Object(),
...
#If DEBUG Then
      If (namedParameters IsNot Nothing) Then
...
#End If
      ...
      Debug.Assert(name IsNot Nothing, "Invalid argument")
      ...
      invokeMethod = BindToMethod(invokeAttr, p, args, Nothing, Nothing, namedParameters, binderState)
      ...
      If (binderState IsNot Nothing) Then
        ReorderArgumentArray(args, binderState)
      End If
      Return res
    ...
    Private Function GetDefaultMemberName(typ As Type) As String
      ...
      Do
        ...
        End If
        typ = typ.BaseType
      Loop While (typ IsNot Nothing)

      Return Nothing

    End Function

    Private Function GetMethodsByName(objType As System.Type, objIReflect As IReflect, name As String, invokeAttr As BindingFlags) As MethodBase()
      ...
      For MemberIndex = 0 To mi.GetUpperBound(0)
        ...
        ElseIf ThisMember.MemberType = MemberTypes.Field Then
          ...
          For j = 0 To mi.GetUpperBound(0)
            ...
            If MemberIndex <> j AndAlso (mi(j) IsNot Nothing) AndAlso
      ...
      For Index As Integer = 0 To mi.Length - 1
        If mi(Index) IsNot Nothing Then
    ...
    Friend Function CalledMethodName() As String
      Debug.Assert((m_objType IsNot Nothing) AndAlso (m_BindToName IsNot Nothing))
    ...
    Friend Shared Sub SecurityCheckForLateboundCalls(member As MemberInfo, objType As Type, objIReflect As IReflect)
      ...
      If (objType IsNot objIReflect) AndAlso (Not IsMemberPublic(member)) Then
    ...
    Private Shared Function IsMemberPublic(Member As MemberInfo) As Boolean
      Debug.Assert(Member IsNot Nothing, "How can this be Nothing ?")

...
Imports Community.VisualBasic.CompilerServices.NewLateBinding
...
Imports Community.VisualBasic.CompilerServices.ReflectionExtensions
...
Namespace Global.Community.VisualBasic.CompilerServices
  ...
  Friend Class Symbols
    ...
    Friend Shared Function GetTypeCode(type As System.Type) As TypeCode
    ...
    Friend Shared Function IsRootObjectType(type As System.Type) As Boolean
      Return type Is GetType(Object)
    End Function

    Friend Shared Function IsRootEnumType(type As System.Type) As Boolean
      Return type Is GetType(System.Enum)
    End Function

    Friend Shared Function IsValueType(type As System.Type) As Boolean
      Return type.IsValueType
    End Function

    Friend Shared Function IsEnum(type As System.Type) As Boolean
      Return type.IsEnum
    End Function

    Friend Shared Function IsArrayType(type As System.Type) As Boolean
      Return type.IsArray
    End Function

    Friend Shared Function IsStringType(type As System.Type) As Boolean
      Return type Is GetType(String)
    End Function

    Friend Shared Function IsCharArrayRankOne(type As System.Type) As Boolean
    ...
    Friend Shared Function IsNumericType(type As System.Type) As Boolean
    ...
    Friend Shared Function IsIntrinsicType(type As System.Type) As Boolean
      Return IsIntrinsicType(GetTypeCode(type)) AndAlso Not IsEnum(type)
    End Function


    Friend Shared Function IsClass(type As System.Type) As Boolean
      Return type.IsClass OrElse IsRootEnumType(type)
    End Function

    Friend Shared Function IsClassOrValueType(type As System.Type) As Boolean
      Return IsValueType(type) OrElse IsClass(type)
    End Function

    Friend Shared Function IsInterface(type As System.Type) As Boolean
      Return type.IsInterface
    End Function

    Friend Shared Function IsClassOrInterface(type As System.Type) As Boolean
      Return IsClass(type) OrElse IsInterface(type)
    End Function

    Friend Shared Function IsReferenceType(type As System.Type) As Boolean
      Return IsClass(type) OrElse IsInterface(type)
    End Function

    Friend Shared Function IsGenericParameter(type As System.Type) As Boolean
...
#If LATEBINDING Then
        ...
        End Function
#Else
    Friend Shared Function IsCollectionInterface(Type As System.Type) As Boolean
      If Type.IsInterface AndAlso
         ((Type.IsGenericType AndAlso
    ...
    End Function
#End If
    Friend Shared Function [Implements](implementor As System.Type, [interface] As System.Type) As Boolean
    ...
    Friend Shared Function IsOrInheritsFrom(derived As System.Type, base As System.Type) As Boolean
...
#If DEBUG Then
    Friend Shared Function IsInstantiatedGeneric(Method As MethodBase) As Boolean
...
#End If
    ...
    Friend Shared Function GetElementType(type As System.Type) As Type
...
parameters As ParameterInfo(),
    ...
    Friend NotInheritable Class Container
      ...
      Private Class InheritanceSorter : Implements IComparer(Of MemberInfo)
        ...
        Private Function Compare(left As MemberInfo, right As MemberInfo) As Integer Implements IComparer(Of MemberInfo).Compare
...
#End If
      ...
      Friend Function LookupWinRTCollectionInterfaceMembers(memberName As String) As List(Of MemberInfo)
        Debug.Assert(IsWindowsRuntimeObject(), "Expected a Windows Runtime Object")
      ...
      Friend Function LookupNamedMembers(memberName As String) As MemberInfo()
        ...
        If IsWindowsRuntimeObject() Then
...
reportErrors As Boolean) As MemberInfo()
        ...
        If memberName = "" Then
          ...
          result = LookupDefaultMembers(memberName, _type) 'MemberName is set during this call.
          ...
          If result.Length = 0 Then
            If reportErrors Then
              Throw New MissingMemberException(
                  SR.Format(SR.MissingMember_NoDefaultMemberFound1, VBFriendlyName))
            End If
        ...
        Else
          result = LookupNamedMembers(memberName)

          If result.Length = 0 Then
            If reportErrors Then
              Throw New MissingMemberException(
                  SR.Format(SR.MissingMember_MemberNotFoundOnType2, memberName, VBFriendlyName))
            End If
      ...
      Friend Sub SetFieldValue(field As FieldInfo, value As Object)
        If field.IsInitOnly Then
          Throw New MissingMemberException(
              SR.Format(SR.MissingMember_ReadOnlyField2, field.Name, VBFriendlyName))
        End If
      ...
      Friend Function GetArrayValue(indices As Object()) As Object
        Debug.Assert(IsArray, "expected array when getting array value")
      ...
      Friend Sub SetArrayValue(arguments As Object())
        ...
        Debug.Assert(IsArray, "expected array when setting array value")
...
flags As BindingFlags) As Object
    ...
    Friend NotInheritable Class Method
      ...
      Private ReadOnly _rawItem As MethodBase                  'The unsubstituted raw generic method.
      Private _parameters As ParameterInfo()          'The parameters used for this method by overload resolution.
      Private ReadOnly _rawParameters As ParameterInfo()       'The unsubstituted raw parameters of a generic method.
      ...
      Private Sub New(
parameters As ParameterInfo(),
paramArrayIndex As Integer,
paramArrayExpanded As Boolean)
        ...
        AllNarrowingIsFromObject = True  'Assume True until non-object narrowing is encountered.
      End Sub
...
paramArrayExpanded As Boolean)
      ...
      Friend Function BindGenericArguments() As Boolean
        ...
        Debug.Assert(ArgumentsValidated, "can't bind without validating arguments")
        Debug.Assert(IsMethod, "binding to a non-method")
        Try
      ...
      Friend Function AsMethod() As MethodBase
        Debug.Assert(IsMethod, "casting a non-method to a method")
        Return TryCast(_item, MethodBase)
      End Function

      Friend Function AsProperty() As PropertyInfo
        Debug.Assert(IsProperty, "casting a non-property to a property")

...
#End If
    ...
    Public Sub FileClose()
      ' TODO Check: Local function was replaced with Lambda
      Dim createAndOpenFile As Action(Of Integer) = Sub(fileNumber1 As Integer)

                                                      Dim fileName = GetTestFilePath()
                                                      IO.File.WriteAllText(fileName, "abc123")
                                                      FileSystem.FileOpen(fileNumber1, fileName, OpenMode.Input)
    ...
    Public Sub FileLen()
      ...
      ' Removed as it is not actually utilized...????
      'fileNumber = FileSystem.FreeFile()
